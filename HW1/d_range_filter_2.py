...
def image_range_filter(img, k_size=3, threshold=30):
    h, w = img.shape
    pad = k_size // 2
    ...
    for i in range(h):
        for j in range(w):
            ...
            if range_val > threshold and (center_val == min_val or center_val == max_val):
                output[i, j] = (int(max_val) + int(min_val)) // 2
            else:
...
def main():
    ...
    output_result = "./output/result_d_range.png"      # 純結果圖 
    output_compare = "./output/compare_d_range.png"    # 比較圖 
