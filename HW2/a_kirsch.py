...
def image_kirsch_filter(img):
    ...
    def kirsch_single_channel(channel):
        h, w = channel.shape
    ...
    if img.ndim == 2:
        response = kirsch_single_channel(img)
    elif img.ndim == 3 and img.shape[2] == 3:
        channels = cv2.split(img)
        responses = [kirsch_single_channel(ch) for ch in channels]
        ...
        raise ValueError("image_kirsch_filter 只接受灰階、BGR 或 BGRA 影像")
...
def main():
    ...
    output1 = "./HW2/output/result_a_kirsch_lenna.png"
    output2 = "./HW2/output/result_a_kirsch_chessking.png"
    output3 = "./HW2/output/result_a_kirsch_breakfast.png"
    compare_output1 = "./HW2/output/compare_a_kirsch_lenna.png"
    compare_output2 = "./HW2/output/compare_a_kirsch_chessking.png"
    compare_output3 = "./HW2/output/compare_a_kirsch_breakfast.png"
    ...
    out1 = image_kirsch_filter(img1)
    out2 = image_kirsch_filter(img2)
    out3 = image_kirsch_filter(img3)
    ...
    cv2.imshow("Lenna Input | Kirsch Output", compare1)
    cv2.imshow("Chessking Input | Kirsch Output", compare2)
    cv2.imshow("Breakfast Input | Kirsch Output", compare3)

    print(f"Kirsch Filter 完成：{output1}")
    print(f"Kirsch Filter 完成：{output2}")
    print(f"Kirsch Filter 完成：{output3}")
    print(f"Kirsch 比較圖完成：{compare_output1}")
    print(f"Kirsch 比較圖完成：{compare_output2}")
    print(f"Kirsch 比較圖完成：{compare_output3}")
