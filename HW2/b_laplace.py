...
def image_laplace_filter(img):
    # 4-neighborhood Laplacian mask
    # mask = np.array([[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=np.float32)
    # mask = np.array([[0, -1, 0], [-1, 4, -1], [0, -1, 0]], dtype=np.float32)
    # 8-neighborhood Laplacian mask
    ...
    if img.ndim == 2:
        ...
        raise ValueError("image_laplace_filter 只接受灰階、BGR 或 BGRA 影像")
    ...
    return response, response_abs, display.astype(np.uint8)


def zero_crossing_detection(laplace_response, threshold=20):
    """
    ...
    Args:
        laplace_response: Laplace 濾波的響應 (浮點數)  
        threshold: T，邊緣幅度門檻值。越大邊緣點越少。建議試試 20~100
    
    Returns:
        edges: 二值影像，邊緣為 255，其他為 0
    """
    h, w = laplace_response.shape
    edges = np.zeros((h, w), dtype=np.uint8)
    
    for i in range(1, h - 1):
        for j in range(1, w - 1):
            center = laplace_response[i, j]
            
            # 檢查 8 個鄰域
            neighbors = [
                laplace_response[i-1, j-1], laplace_response[i-1, j], laplace_response[i-1, j+1],
                laplace_response[i,   j-1],                           laplace_response[i,   j+1],
                laplace_response[i+1, j-1], laplace_response[i+1, j], laplace_response[i+1, j+1]
            ]
...
def main():
    ...
    output1 = "./HW2/output/b_result_b_laplace_lenna.png"
    # output2 = "./HW2/output/b_result_b_laplace_chessking.png"
    # output3 = "./HW2/output/b_result_b_laplace_breakfast.png"
    compare_output1 = "./HW2/output/b_compare_b_laplace_lenna.png"
    ...
    laplace_raw1, laplace_abs1, out1 = image_laplace_filter(img1)
    # laplace_raw2, laplace_abs2, out2 = image_laplace_filter(img2)
    # laplace_raw3, laplace_abs3, out3 = image_laplace_filter(img3)
    ...
    # 保存 raw 和 abs 版本
    raw_output1 = output1.replace("b_result_", "b_raw_")
    abs_output1 = output1.replace("b_result_", "b_abs_")
    
    # Raw: 有符號響應（將負值移到 128 顯示中灰）
    raw_display = np.clip(laplace_raw1 + 128.0, 0, 255).astype(np.uint8)
    cv2.imwrite(raw_output1, raw_display)
    
    # Abs: 絕對值（已經在 out1 中，但也單獨存一份）
    cv2.imwrite(abs_output1, laplace_abs1)

    # 對每個 T 值執行 Zero Crossing Edge Detection 並存檔
    for T in threshold_values:
        edges = zero_crossing_detection(laplace_raw1, threshold=T)
        results[T] = edges
        
        edge_output = output1.replace("b_result_", f"b_edge_T{T}_")
    ...
    compare_parts = [img1, out1_bgr]  # 先放原圖和 Laplace 絕對值
    ...
    can_show = True
    try:
        cv2.imshow("Lenna Input | Laplace Output", compare1)
        # cv2.imshow("Chessking Input | Laplace Output", compare2)
        # cv2.imshow("Breakfast Input | Laplace Output", compare3)
    except cv2.error:
        can_show = False

    # 統計基本資訊
    min_response = int(np.min(laplace_raw1))
    max_response = int(np.max(laplace_raw1))
    
    print(f"\nLaplace Filter 完成：{output1}")
    print(f"Laplace Raw 版本（有符號）：{raw_output1}")
    print(f"Laplace Abs 版本（絕對值）：{abs_output1}")
    print(f"Laplace 比較圖完成：{compare_output1}")
    print(f"Laplace 原始響應範圍：{min_response} 到 {max_response}")
    
    # 保存統計結果到文本檔案
    stats_output = output1.replace("b_result_", "b_stats_").replace(".png", ".txt")
    with open(stats_output, "w", encoding="utf-8") as f:
        f.write("=== Laplace Edge Detection Statistics ===\n\n")
        f.write(f"Input Image: {input1}\n\n")
        f.write(f"Laplace Response Range: {min_response} ~ {max_response}\n")
